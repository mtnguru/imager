(function ($) {
  ...
  $.imgAreaSelect = function (img, options) {
    var $img = $(img), imgLoaded, $box = div(), $area = div(), $border = div().add(div()).add(div()).add(div()), $outer = div().add(div()).add(div()).add(div()), $handles = $([]), $areaOpera, left, top, imgOfs = {
      left: 0,
      top: 0
    }, imgWidth, imgHeight, $parent, parOfs = {
      left: 0,
      top: 0
    }, zIndex = 0, position = 'absolute', startX, startY, scaleX, scaleY, resize, minWidth, minHeight, maxWidth, maxHeight, aspectRatio, shown, x1, y1, x2, y2, selection = {
      x1: 0,
      y1: 0,
      x2: 0,
      y2: 0,
    ...
    function areaMouseMove(event) {
      ...
      if (options.resizable) {
        if (y <= options.resizeMargin) {
          ...
          if (y >= selection.height - options.resizeMargin) {
            resize = 's';
          }
        }
        if (x <= options.resizeMargin) {
          ...
          if (x >= selection.width - options.resizeMargin) {
            resize += 'e'
          }
        }
      }
    ...
    function imgMouseDown(event) {
      if (event.which != 1 || $outer.is(':animated')) {
        return false;
      }
      adjust();
      startX = x1 = evX(event);
    ...
    var msie = (/msie ([\w.]+)/i.exec(ua) || [])[1], opera = /opera/i.test(ua), safari = /webkit/i.test(ua) && !/chrome/i.test(ua);
    $p = $img;
    while ($p.length) {
